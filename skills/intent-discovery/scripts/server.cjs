...
// Ascension port: the companion never loads an external brand image and never
// phones home. Branding is local text only; no telemetry exists to disable.
const SUPERPOWERS_TELEMETRY_DISABLED = true;
...
function brandMarkup() {
  ...
  const text = 'Ascension Visual Brainstorming v' + version;
  // No external logo image — local text branding only, no network requests.
